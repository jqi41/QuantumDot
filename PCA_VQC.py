...
#### Using PCA from the sklearn library
...
class PCA_VQC(nn.Module):
    def __init__(self):
        ...
        self.vqc = VQC(n_wires=8, n_qlayers=2)
    
    def forward(self, x):
        self.dev = tq.QuantumDevice(n_wires=8, bsz=x.shape[0])
        x = self.flatten(x)
...
if __name__ == "__main__":
    ...
    if args.test_kind == 'rep':
        ...
        TypeError("Only 'rep' and 'gen' are supported!")
    ...
    optimizer = torch.optim.Adam(pca_vqc.parameters(), lr=args.lr) # The optimizer is Adam
    ...
    for t in range(args.num_epochs):
        print(f"Epoch {t+1}\n --------------------")
        train(trainloader, pca_vqc, loss_fn, optimizer, train_loss_pca_vqc, train_acc_pca_vqc)
        test(testloader, pca_vqc, loss_fn, test_loss_pca_vqc, test_acc_pca_vqc)
    print("Done!")

    create_acc_loss_graph(train_acc_pca_vqc, train_loss_pca_vqc, test_acc_pca_vqc, test_loss_pca_vqc)
