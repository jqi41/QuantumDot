...
"""Charge stability diagram classification with Pre-X+VQC models. The Pre-X+NN includes:
    1. (PreTrained)-ResNet18 + NN
    2. (PreTrained)-ResNet50 + NN
...
class PostNN(nn.Module):
    def __init__(self, num_qubits=8):
        ...
        self.vqc = nn.Linear(num_qubits, 2)
    
    def forward(self, x):
        x = self.flatten(x)
        pre_out = self.pre_net(x) 

        q_in = torch.sigmoid(pre_out)
        q_out = self.vqc(q_in)
...
if __name__ == "__main__":
    ...
    ### 2. Set up the model 
    if args.model_kind == 'ResNet50':
        PreX_nn = torchvision.models.resnet50(weights=torchvision.models.ResNet50_Weights.IMAGENET1K_V2)
        print('Building up a Pre-ResNet50+NN model:')
    elif args.model_kind == 'ResNet18':
        print('Building up a Pre-ResNet18+NN model:')
        #PreX_vqc = torchvision.models.resnet18(weights=torchvision.models.ResNet18_Weights.IMAGENET1K_V1)
        PreX_nn = torchvision.models.resnet18()
    else:
        TypeError("The pre-trained model should be ResNet18 or ResNet50")
        
    for param in PreX_nn.parameters():
        param.requires_grad = False
    PreX_nn.conv1 = torch.nn.Conv2d(1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
    PreX_nn.fc = PostNN(args.num_qubits)
    PreX_nn = PreX_nn.to(device)
    ...
    optimizer = torch.optim.Adam(PreX_nn.parameters(), lr=args.lr) # The optimizer is Adam
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.5) # For each 10 iterations, the lr is divided by 2
    ...
    for t in range(args.num_epochs):
        print(f"Epoch {t+1}\n --------------------")
        train(trainloader, PreX_nn, loss_fn, optimizer, train_loss_PreX_nn, train_acc_PreX_nn)
        test(testloader, PreX_nn, loss_fn, test_loss_PreX_nn, test_acc_PreX_nn)
        scheduler.step()
    print("Done!")

    create_acc_loss_graph(train_acc_PreX_nn, train_loss_PreX_nn, test_acc_PreX_nn, test_loss_PreX_nn)
